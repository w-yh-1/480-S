...
public:
    encoder(RK_U32 width, RK_U32 height, MppFrameFormat frame_fmt = MPP_FMT_BGRA8888, MppCodingType type = MPP_VIDEO_CodingAVC, MppEncRcMode rc_mode = MPP_ENC_RC_MODE_VBR);
    ~encoder();
    MPP_RET encode(HyBirdBuffer *input_CamBuffer);
    MPP_RET init();
    MPP_RET cfg_setup();
    MPP_RET encode_input();
    void encode_commit_input_buf(rga_buffer_t *rga_buf=NULL);
    friend void *encode_input_thread(void *arg);
    friend void *encode_output_thread(void *arg);
    friend void *transcode_thread(void *arg);
    void deinit();
    void launch(pOutput_func p,unsigned int* osd_gain_val, unsigned int* osd_count_val, unsigned int* osd_count_period, unsigned int* osd_distance_val);
    void stop_record();
    void pause_record();
    bool start_record(char *save_path);
    HyBirdBuffer mEnc_buf;
    MppFrame frame = NULL;
    MppEncOSDData  osd_data;
    unsigned int *osd_gain_val=NULL;
    unsigned int *osd_count_val=NULL;
    unsigned int *osd_distance_val=NULL;
    unsigned int *osd_count_period=NULL;
    static bool isCN;
    ...
    static void draw_cn_osd(char *ptr, int DW_per_line = 8);
    static void draw_en_osd(char *ptr, int DW_per_line = 8);
};
