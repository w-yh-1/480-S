...
typedef struct
{
    ...
    float relHum;
    float distance;
    float reflectedTemper;
    float atmosphericTemper;
    float modifyK;
    float modifyB;
 ...
 * @return      @ref gmt_ret_code_e  0 成功 <0 失败
 */
dllexport int guide_measure_convertgray2temper(int devType, int lensType, short* pGray,  unsigned char* pParamLine, int len, guide_measure_external_param_t* pParamExt, float* pTemper);
/**
 * @brief 将温度数据转换成Y16数据
 * @param[in]   devType     机芯型号。
 * @param[in]   lensType    镜头类型。
 ...
 * @return      @ref gmt_ret_code_e  0 成功 <0 失败
 */
dllexport int guide_measure_converttemper2gray(int devType, int lensType, float *pTemper,  unsigned char* pParamLine, int len, guide_measure_external_param_t *pParamExt, short *pGray);
...
dllexport int guide_isotherm(int devtype, int lenstype,float temperal, float temperah, short* y16Data, short* yuvsrcData,short* yuvdstData, unsigned char* paramline, int width, int height, guide_measure_external_param_t* pParamExt, guide_isothermmode_e isothermmode,int paletteIndex);
